...
def load_model(model_checkpoint, repo_name):
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        max_steps=10000,                 # ( int, 선택 사항 , 기본값은 -1) – 양수로 설정된 경우 수행할 총 훈련 단계 수
...
if __name__ == '__main__':
    ...
    repo_name = f"{model_checkpoint.split('/')[1]}_{train_dataset_name.split('/')[-1]}_test" 
    ...
    trainer.train(resume_from_checkpoint=f"whisper_compare\whisper-small_child-50k2\checkpoint-8000")
    ...
    prediction = trainer.predict(test_dataset["test"])
    with open(f"D:/stt/checkPoints/{repo_name}/test_results.txt", "w") as f:
        f.write(str(prediction.metrics))
    print(f"Test result: {prediction.metrics}")
    
    # push to hub
    push_to_hub("child-50k", model_checkpoint, trainer)
