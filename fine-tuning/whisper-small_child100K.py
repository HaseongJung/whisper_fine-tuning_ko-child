...
def prepare_dataset(dataset, feature_extractor, tokenizer, num_cores):
    ...
    dataset = dataset.map(partial_prepare_dataset, remove_columns=dataset.column_names["train"], num_proc=num_cores)
...
def load_model(model_checkpoint, repo_name):
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        per_device_eval_batch_size=2,          #  ( int, 선택 사항 , 기본값은 8) – 평가를 위한 GPU/TPU 코어/CPU당 배치 크기
...
if __name__ == '__main__':
    ...
    # load dataset
    dataset_name = "haseong8012/child-100k"
    ...
    repo_name = f"{model_checkpoint.split('/')[-1]}_{dataset_name.split('/')[-1]}" 
